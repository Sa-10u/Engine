...
void D3D::Shader_InitializeToon3D()
{
    HRESULT hr = S_OK;
    ...
    assert(pCompileVS != nullptr);
    pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shader_bundle[static_cast<int>(SHADER_TYPE::SHADER_TOON3D)].vs);
    ...
    pDevice_->CreateInputLayout(layout, (sizeof(layout) / sizeof(layout[0])), pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shader_bundle[static_cast<int>(SHADER_TYPE::SHADER_TOON3D)].lo);

    SAFE_RELEASE(pCompileVS);


    ID3DBlob* pCompilePS = nullptr;
    D3DCompileFromFile(L"Toon3D.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
    assert(pCompilePS != nullptr);
    pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shader_bundle[static_cast<int>(SHADER_TYPE::SHADER_TOON3D)].ps);
    SAFE_RELEASE(pCompilePS);
